...
MPHBAdmin.CalendarPopup = can.Control.extend(
    {},
    {
        ...
        ajaxUrl: '',
        ajaxAction: 'mphb_get_booking_info',
        ajaxNonce: '',
        ...
        loadBookingDetails: function (bookingId) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    // Push new HTML or error message
                    if (response.hasOwnProperty('success')) {
                        if (response.success) {
                            self.updateContent(response.data.bookingInfo);
                        } else {
		...
		$.each( ctrls, function() {
			...
			switch ( type ) {
				...
				case 'price-breakdown':
					...
					break;
				case 'gallery':
					ctrl = new MPHBAdmin.GalleryCtrl( $( this ) );
					break;
				case 'datepicker':
		...
		wp.media.controller.MPHBGallery = wp.media.controller.FeaturedImage.extend( {
			...
			updateSelection: function() {
				var selection = this.get( 'selection' ),
					ids = MPHBAdmin.WPGallery.myThis.ctrl.get(),
					attachments;
				if ( '' !== ids && -1 !== ids ) {
 ...
 * @requires ./ctrl.js
 */
MPHBAdmin.GalleryCtrl = MPHBAdmin.Ctrl.extend( {}, {
	input: null,
	previewHolder: null,
	addGalleryBtn: null,
